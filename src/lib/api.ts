...
export const api = {
  certificates: {
    ...
    getAll: async (page = 1, limit = 10): Promise<{ data: Certificate[]; pagination: { page: number; limit: number; total: number; totalPages: number } }> => {
      const response = await fetch(`/api/certificates?page=${page}&limit=${limit}`, { cache: 'no-store' });
      if (!response.ok) {
    ...
    getById: async (id: string): Promise<Certificate> => {
      ...
      const response = await fetch(`/api/certificates?page=1&limit=1000`, { cache: 'no-store' });
      if (!response.ok) {
        throw new Error('Failed to fetch certificates');
      }
      const result = await response.json();
      ...
      const found = certificates.find((c: ApiCertificate) => String(c.id) === String(id));
