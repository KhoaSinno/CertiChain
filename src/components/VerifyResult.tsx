...
import { Calendar, CheckCircle, Copy, ExternalLink, Eye, GraduationCap, ShieldCheck, User, XCircle } from 'lucide-react';
        ...
        <Card className="border-red-200/50 bg-red-50/60 supports-[backdrop-filter]:bg-red-50/40 backdrop-blur-md">
          <CardContent className="py-8">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <h3 className="text-2xl font-extrabold text-red-700">Chứng chỉ không hợp lệ</h3>
              <p className="text-sm text-red-600/80">
                Không tìm thấy thông tin tương ứng với mã đã nhập. Vui lòng kiểm tra lại transaction hash.
              </p>
              <div className="mt-3 text-xs text-muted-foreground">
                Đã kiểm tra hash:
                <code className="ml-2 px-2 py-1 bg-white/70 rounded font-mono">{hash}</code>
              </div>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className={`w-full ${className}`}>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h4 className="text-lg font-bold leading-tight">Chứng chỉ hợp lệ</h4>
            </div>
          </div>
      ...
      <Card className="bg-background/60 supports-[backdrop-filter]:bg-background/40 backdrop-blur-md border border-border/40 shadow-lg">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <Button onClick={handleViewCertificate} size="sm" className="gap-1.5">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-muted/50 rounded-lg p-3">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Button variant="ghost" size="sm" onClick={() => navigator.clipboard.writeText(hash)} className="h-7 px-2 text-xs gap-1">
                <Copy className="h-3 w-3" />
                Copy
              </Button>
            </div>
            <code className="text-xs text-muted-foreground break-all block">{hash}</code>
          </div>
            ...
            <div className="rounded-lg p-3 bg-green-600 text-white shadow-md">
              <div className="flex items-start gap-2.5 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-green-50 mt-0.5">Giao dịch được xác nhận và lưu trữ vĩnh viễn</p>
                </div>
              </div>
              <div className="bg-white rounded-md p-2">
                <p className="text-[10px] text-muted-foreground mb-0.5">Transaction Hash:</p>
                <code className="text-[10px] font-mono text-green-700 break-all block">{certificate.transactionHash}</code>
              </div>
            </div>
          )}
        </CardContent>
      ...
      {(certificate.ipfsHash || certificate.transactionHash) && (
        <div className={`grid gap-2 ${certificate.ipfsHash && certificate.transactionHash ? 'grid-cols-2' : 'grid-cols-1'}`}>
          {certificate.ipfsHash && (
            <Button variant="outline" size="sm" onClick={() => window.open(`https://ipfs.io/ipfs/${certificate.ipfsHash}`, '_blank')} className="transition-transform hover:scale-105 active:scale-95">
              <ExternalLink className="h-3.5 w-3.5 mr-1.5" />
              Xem file gốc
            </Button>
          )}
          {certificate.transactionHash && (
            <Button variant="outline" size="sm" onClick={() => window.open(`https://sepolia.basescan.org/tx/${certificate.transactionHash}`, '_blank')} className="transition-transform hover:scale-105 active:scale-95">
              <ExternalLink className="h-3.5 w-3.5 mr-1.5" />
              BaseScan
            </Button>
          )}
      ...
      <div className="mt-4 text-center">
        <p className="text-[10px] text-muted-foreground">Kết quả xác minh được lấy từ blockchain Base Sepolia</p>
      </div>
    </div>
  );
}
